...
class mymodel(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit = timm.create_model('swin_base_patch4_window7_224_in22k', pretrained=True, num_classes = 14)
    def forward(self, x):
        x = self.vit(x)
        return x
...
for epoch in range(epochs):
    ...
    if test_accuracy / len(test_loader) * 100 > best_accuracy:
        ...
        torch.save(model.state_dict(), "./swin_base_patch4_window7_224_in22k.pth")
        print("Save model")
    print("=================================================")
...
plt.savefig('swin_base_patch4_window7_224_in22k.png')


# %%

# Load weight with best accuracy
model.load_state_dict(torch.load("./swin_base_patch4_window7_224_in22k.pth"))
