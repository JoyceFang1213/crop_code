...
class mymodel(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit = timm.create_model('xcit_small_24_p16_384_dist', pretrained=True, num_classes = 14)
    def forward(self, x):
        x = self.vit(x)
        return x
...
for epoch in range(epochs):
    ...
    if test_accuracy / len(test_loader) * 100 > best_accuracy:
        ...
        torch.save(model.state_dict(), "./xcit_small_24_p16_384_dist.pth")
        print("Save model")
    print("=================================================")
...
plt.savefig('xcit_small_24_p16_384_dist.png')


# %%

# Load weight with best accuracy
model.load_state_dict(torch.load("./xcit_small_24_p16_384_dist.pth"))
