...
class mymodel(nn.Module):
    def __init__(self):
        super().__init__()
        self.vit = timm.create_model('gluon_resnet101_v1s', pretrained=True, num_classes = 14)
    def forward(self, x):
        x = self.vit(x)
        return x
...
for epoch in range(epochs):
    ...
    if test_accuracy / len(test_loader) * 100 > best_accuracy:
        ...
        torch.save(model.state_dict(), "./gluon_resnet101_v1s_384.pth")
        print("Save model")
    print("=================================================")
...
plt.savefig('gluon_resnet101_v1s_384.png')


# %%

# Load weight with best accuracy
model.load_state_dict(torch.load("./gluon_resnet101_v1s_384.pth"))
